...
#' Supports both synthetic data (from generate_data.R) and processed raw data
load_complete_data <- function() {
  
  # First, try to load processed raw data (from process_raw_data.py)
  processed_file <- file.path(DATA_PROCESSED_DIR, "survey_processed.csv")
  if (file.exists(processed_file)) {
    data <- read_csv(processed_file, show_col_types = FALSE)
    message(paste("Loaded", nrow(data), "records from processed raw data"))
    return(data)
  }
  ...
  if (file.exists(file_path)) {
    ...
    message(paste("Loaded", nrow(data), "records from synthetic dataset"))
    return(data)
  }
  
  stop("No data file found. Please run process_raw_data.py or 01_generate_data.R first.")
}
...
clean_survey_data <- function(data) {
  ...
  month_levels <- if (all(grepl("^M\\d+$", unique_months))) {
    sort(unique_months)
  } else {
    paste0("M", sprintf("%02d", 1:N_MONTHS))
  }
  
  # Convert factors - handle cases where columns may not exist
  data <- data %>%
    mutate(
      region = factor(region, levels = c(REGIONS, "Unknown")),
      segment = factor(segment, levels = c(DEMOGRAPHIC_SEGMENTS, "Unknown")),
      month = factor(month, levels = month_levels),
      gender = factor(gender),
      relationship_status = if ("relationship_status" %in% names(.)) factor(relationship_status) else factor("Unknown"),
      income_bracket = if ("income_bracket" %in% names(.)) {
        factor(income_bracket, levels = c("Under 3M", "3-5M", "5-7M", "7-10M", "Over 10M", "Unknown"))
      } else factor("Unknown")
    )
...
create_derived_variables <- function(data) {
  ...
  data <- data %>%
    mutate(
      # Month number for time trends - handle different formats
      month_num = as.numeric(gsub("M", "", as.character(month))),
      
      # Age groups - handle NA values
      age_group = if ("age" %in% names(.) && !all(is.na(age))) {
        cut(age, 
